...
namespace CourseLibrary.API.ActionConstraints
{
    ...
    public class RequestHeaderMatchesMediaType : Attribute, IActionConstraint
    {
        ...
        {
            IHeaderDictionary requesHeaders = context.RouteContext.HttpContext.Request.Headers;
            if (!requesHeaders.ContainsKey(_requestHeaderToMatch))
            {
                return false;
            }

            var parsedRequestMediaType = new MediaType(requesHeaders[_requestHeaderToMatch]);
